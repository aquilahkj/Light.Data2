...
{
    public class Mssql_RelateModeTest : BaseTest
    {
        public Mssql_RelateModeTest(ITestOutputHelper output) : base(output)
        {
        }
        ...
        #region version
        [Fact]
        public void TestCase_Base_PageSkipTake_Ver2012()
        {
...

            {
                ...
                var listAc = context.Query<TeRelateA_B>().Where(x => x.Id > cnt).Skip(cnt).Take(cnt).ToList();
                AssertExtend.StrictEqual(listEx, listAc);
            }
